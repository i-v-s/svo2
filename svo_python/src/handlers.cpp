...
void initHandlers(py::module &m)
{
    ...
    py::class_<FHMWrapper, shared_ptr<FHMWrapper>>(m_handler, "HandlerMono")
...
FHMWrapper::FHMWrapper(std::shared_ptr<vk::AbstractCamera> cam, std::shared_ptr<vilib::DetectorBaseGPU> detector, size_t pyramidLevels):
    svo::FrameHandlerMono(cam, detector), pyramidLevels(pyramidLevels), camera(cam)
{

}

void FHMWrapper::addImage(at::Tensor tensor, double timestamp) {
    std::unique_ptr<svo::Frame> frame;
    {
